...
impl DynamicColumnHandle {
    // TODO rename load
    pub async fn open(&self) -> io::Result<DynamicColumn> {
        let column_bytes: OwnedBytes = self.file_slice.read_bytes()?;
        self.open_internal(column_bytes).await
    }
    ...
    /// FastValue.
    pub async fn open_u64_lenient(&self) -> io::Result<Option<Column<u64>>> {
        let column_bytes = self.file_slice.read_bytes()?;
        match self.column_type {
            ColumnType::Str | ColumnType::Bytes => {
                let column: BytesColumn =
                    crate::column::open_column_bytes(column_bytes, self.format_version).await?;
    ...
    async fn open_internal(&self, column_bytes: OwnedBytes) -> io::Result<DynamicColumn> {
        let dynamic_column: DynamicColumn = match self.column_type {
            ...
            ColumnType::Str => crate::column::open_column_str(column_bytes, self.format_version)
                .await?
                .into(),
            ColumnType::I64 => {
                crate::column::open_column_u64::<i64>(column_bytes, self.format_version)
                    .await?
                    .into()
            }
            ColumnType::U64 => {
            ...
            ColumnType::F64 => {
                crate::column::open_column_u64::<f64>(column_bytes, self.format_version)
                    .await?
                    .into()
            }
            ColumnType::Bool => {
                crate::column::open_column_u64::<bool>(column_bytes, self.format_version)
                    .await?
                    .into()
            }
            ColumnType::IpAddr => {
